...
The bridge between them - the CNN's continuous embedding and the binary code FLPSI consumes - is
this project's main technical contribution: **the binariser is the weak link, and it can be
co-designed with the private-matching layer.** The FLPSI accept rule is a function of the code's
impostor-Hamming distribution, so one better code lowers both error and communication at once.
We started from a training-free Super-Bit quantiser, then improved the binariser twice:

- **Round 1 - metric-aware ortho-thermometer** (training-free). Super-Bit preserved *cosine*
  distance, but the embedding is ~3.2x more discriminative under *Euclidean*. An orthonormal
  projection + magnitude-threshold code preserves the right geometry: EER 1.89% -> 0.87%.
- **Round 2 - quantization-aware feature-head** (learned). A 128-bit code trained on the CNN's
  richer conv features to be balanced and decorrelated, driving the impostor code toward the
  uniform ideal the FLPSI analysis assumes: EER 0.87% -> 0.17% and 45% less communication.
